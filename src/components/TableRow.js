                ...
                tableRowsData?.map((tableRowData, index) =>
                    {
                        ...
                        else if(index === 4 && !isNaN(parseInt(tableRowsData[3]))) return <td key={index} style={{ color: '#3e6051'}}><p title='Todo Ok' className={`bg-[#c4fcdc] border-white border-solid border-[1px] rounded-[32px] font-semibold  m-1 py-[2px] text-center cursor-pointer ${tableRowData === 'SALIDA' ? 'bg-red-400 text-white' : ''}`}>{tableRowData}</p></td>
                        else return <td className={index === 0 ? 'sticky left-0' : ''} key={index}><p>{tableRowData}</p></td>
